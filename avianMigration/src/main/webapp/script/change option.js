...
function inputRadioChange(evt) {
    ...
    for (var i = 0; i < node.childNodes.length; i++) {
        ...
        if (node.childNodes[i].type === "radio" && node.childNodes[i].checked) {
            ...
            if(node.childNodes[i].id !== 'bpchlt' && node.id !== 'bs')
                document.getElementById('noOfBirdsDiv').style.display = 'none';
                      
            if(node.childNodes[i].id === 'sci_radio' || node.childNodes[i].id === 'comm_radio' || node.childNodes[i].id === 'tax_radio')
            {
                //console.log("I'm here");
                getBirdJSON(node.childNodes[i].id);
            }
           
            if(node.childNodes[i].id === 'equals_sign')
            {       
...
function getBirdJSON(id)
{
    ...
    $.getJSON('/avianMigration/GetBirdData', {birdData: true, id: id}, function(data) {      
